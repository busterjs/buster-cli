...
module.exports = {
    ...
    onConfig: function (callback) {
        ...
        config.filterEnv(this.envFilter || this.configEnv.value);
...
function noGroupsError(file, groups) {
    if (this.configGroup.value) {
        return {
            message: file + " contains no configuration groups that " +
                "matches '" + this.configGroup.value + "'\n" +
                "Try one of:\n  " +
                groups.map(function (g) { return g.name; }).join("\n  ")
        };
    }

    if (this.envFilter || this.configEnv.value) {
        return {
            message: file + " contains no configuration groups for " +
                "environment '" + (this.envFilter || this.configEnv.value) + "'\n" +
                "Try one of: " +
                groups.map(function (g) { return g.environment; }).join(", ")
        };
    }

    return { message: file + " contains no configuration groups." };
}
