...
module.exports = {
    ...
    onConfig: function (callback) {
        ...
        try {
            loaded = loadConfigModule(config, this.config.value, "buster.js",
        ...
        config.resolveGroups(function (err) {
            if (err) {
                ...
                callback(null, this.config.value);
            }
        }.bind(this));
    }
};
...
function loadConfigModule(config) {
    ...
    for (var i = 0, l = files.length; i < l; ++i) {
        ...
        try {
            if (config.loadGroupsFromConfigFile(files[i], process.cwd())) return true;
        } catch (e) {
