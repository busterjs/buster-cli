    ...
    opt: function (shortFlag, longFlag, helpText, options) {
        ...
        if ("validators" in options) {
            for (var validatorName in options.validators) {
                ...
                // Some validators take multiple error msgs. Pass in errorMsg twice
                // as a temporary hack to account for this.
                opt.addValidator(busterArgs.validators[validatorName](errorMsg, errorMsg));
            }
        }
