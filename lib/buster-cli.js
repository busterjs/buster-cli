...
module.exports = {
    ...
    loadConfig: function () {
        this.configFile = findConfigFile(this.config.value, process.cwd());
        var config = loadConfig.call(this, this.configFile);

        this.allGroups = config.groups;
    ...
    onConfig: function (callback) {
        try {
            this.resolveConfiguration(this.loadConfig(), callback);
        } catch (e) {
            return callback(e);
        }
    },

    resolveConfiguration: function (config, callback) {
        config.resolveGroups(function (err) {
            if (err) {
                callback(typeof err == "string" ? { message: err } : err);
            } else {
                if (this.config.value.length == 0) {
                    callback(noGroupsError.call(this, this.configFile, this.allGroups));
                } else {
...
function loadConfig(configFile) {
    ...
    if (!configFile) {
        throw { message: this.config.signature + " not provided, and " +
